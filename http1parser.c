/**
  Simple HTTP 1.0/1.1 parser with chunked support. Nothing else.
  */
...
int http_parse(http_response_t * response)
{
    while (response->buffer_index < response->buffer_size)
    {
        ...
        int newline = (response->state != HS_PAYLOAD &&
                        response->buffer[response->buffer_index] == '\n' &&
                        response->last_byte == '\r');
        if (newline && response->buffer_index >= 4 && response->buffer[response->buffer_index - 2] == 0)
        {
            response->payload = response->buffer + response->buffer_index + 1;
            response->state = HS_PAYLOAD;
        }

        if (response->state == HS_PAYLOAD)
        {
...
int main()
{
    ...
    SocketSendHttp(0, &response, 12);
    printf("state: %d\n", response.state);
    printf("code: %d\n", response.response_code);
    for (j = 0; j < response.header_count; ++j)
        printf("%s -> %s\n", response.headers[j].tag, response.headers[j].value);
    printf("payload -> %s\n", response.payload);
    printf("---------------\n");
    return 0;
}
