...
//				printf("add %d on %d\n", ctx.clients[i].socket_fd, count);
				...
				poll_fds[count].events = POLLIN | POLLRDHUP | POLLHUP;
			}
		}

		printf("poll ... %d %d\n", count + 1,  ctx.count);
		int nfd = poll(poll_fds, count + 1, 10000);
		if (nfd < 0)
		{
		...
		for (int i = 0; i < count + 1; ++i)
		{
			...
			if (fd == server_socket) // server sockets
			{
				...
				if (revent & POLLIN)
				{
					...
					{
						fprintf(stderr, "failed to accept: %d %s.\n", errno, strerror(errno));
					}
				...
				if (revent & POLLIN)
				{
					...
					if (read_size > 0)
					{
						for (int j = 0; j < ctx.count; ++j)
						{
							if (ctx.clients[j].socket_fd != fd)
							{
								send(ctx.clients[j].socket_fd, buffer, read_size, 0);
							}
						}
					}
					else if (read_size == 0)
